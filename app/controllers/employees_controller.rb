  ...
  def destroy
    begin
      if @employee.other_manager.present? || @employee.budget_holders.present? || @employee.foreman.present? ||
          Foreman.find_by_id(employee_type_id: @employee.id) || BudgetHolder.find_by_id(employee_type_id: @employee.id) ||
          OtherManager.find_by_id(employee_type_id: @employee.id)
        respond_to do |format|
          format.js
        end
      else
